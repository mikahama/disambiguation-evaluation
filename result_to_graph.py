...
with PdfPages('results_feb_18_2019.pdf') as pdf:
    ...
    for test_lang in lang:
        ...
        target_a = r'[a-zA-Z]{{3}} [a-zA-Z]{{3}}-{x}-4568_\d.json'.format(x=test_lang)
        target_b = r'.*{x}-4568.*.json'.format(x=test_lang)

        root = "./results"
        paths = [os.path.join(root,path) for path in os.listdir(root) if (re.match(target_a, path) or re.match(target_b, path))]

        data = {}
        for tl in train_langs:
            valid_paths = [p for p in paths if os.path.basename(p).startswith(tl + "-")]
            if len(valid_paths) > 0:
                data[tl] = {}
            print(tl, len(valid_paths))
